...
WITH ServerRoleHierarchy AS (
    ...
    SELECT 
        ...
        CAST(RolePath + ' > ' + sp_role.name AS NVARCHAR(MAX)) AS RolePath
    FROM sys.server_role_members srm
    JOIN sys.server_principals sp_role ON srm.role_principal_id = sp_role.principal_id
...
SELECT 
    ...
    perm.major_id,
    ISNULL(srh.RolePath, '') AS ServerRoles,
...
LEFT JOIN ServerRoleHierarchy srh
    ON sp.principal_id = srh.member_principal_id
LEFT JOIN sys.dm_exec_sessions ses
    ON sp.sid = ses.security_id
WHERE sp.type IN ('S', 'U', 'G') -- S=SQL Login, U=Windows Login, G=Windows Group
GROUP BY sp.name, sp.type_desc, sp.default_database_name, sp.create_date, sp.modify_date, perm.class_desc, perm.permission_name, perm.state_desc, perm.major_id, srh.RolePath
ORDER BY sp.name;
...
BEGIN
    ...
    WITH DBRoleHierarchy AS (
        ...
        SELECT 
            drm.member_principal_id,
            drm.role_principal_id,
            CAST(RolePath + '' > '' + drp_role.name AS NVARCHAR(MAX)) AS RolePath
        FROM sys.database_role_members drm
        JOIN sys.database_principals drp_role ON drm.role_principal_id = drp_role.principal_id
    ...
    SELECT 
        ...
        perm.state_desc AS PermissionState,
        perm.major_id,
        ISNULL(drh.RolePath, '''') AS DatabaseRoles
    FROM sys.database_principals dp
    LEFT JOIN sys.database_permissions perm
        ON dp.principal_id = perm.grantee_principal_id
    LEFT JOIN DBRoleHierarchy drh
        ON dp.principal_id = drh.member_principal_id
